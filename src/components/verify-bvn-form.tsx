...
export function VerifyBvnForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch("/api/kyc/verify-bvn", {
      ...
      body: JSON.stringify({ bvn: formData.get("bvn") }),
    });
    ...
    if (!res.ok || !data?.success) {
      setError(data?.message ?? data?.error ?? "We couldn't verify that BVN. Double-check the number and try again.");
      return;
    }
