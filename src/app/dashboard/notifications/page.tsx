...
function ticketPathForRole(role: UserRole, ticketId: string): string {
  if (role === "admin" || role === "support") return `/dashboard/support/tickets/${ticketId}`;
  if (role === "landlord") return `/dashboard/landlord/tickets/${ticketId}`;
  return `/dashboard/tenant/tickets/${ticketId}`;
}
      ...
      <div className="mt-8 flex flex-col gap-3">
        {mine.map((notification) => {
          const href = notification.ticketId
            ? ticketPathForRole(session.user.role, notification.ticketId.toString())
